...
@Component({
  selector: 'app-root',
  imports: [CommonModule, ButtonModule, CardModule, ChipModule, DividerModule, SelectModule, TagModule],
  templateUrl: './app.html',
  styleUrl: './app.scss'
})
export class App {
  protected readonly title = signal('La Casa del Perrocalentero');
  ...
  protected readonly products: ProductCard[] = [
    {
      ...
      price: '$4.50',
      description:
        'Paquete mayorista de 20 unidades de pan suave tradicional. Ideal para carritos y eventos.',
      imageUrl:
    ...
    },
    {
      ...
      price: '$25',
      description:
        '50 panes tradicionales, 50 salchichas tipo viena, 1 kg de papas fritas y 3 salsas.',
      imageUrl:
        'https://lh3.googleusercontent.com/aida-public/AB6AXuBzDseBhHu9EwvRAco9ePAKN5P7xc9lx9fVoTnM1edHkFtu_nDMQr2z-_cRfekhrcHfOsJ_W_buOJqgVb_OLp4UoqKPBEVhA6IMMxYyEzkCUXOxGEbY5hFUMU0uelIITl_FMyarwcgYh9jWX3voMm8b7UAZD56M8Lp1wv_G-ELO4GrfqtnS5IKepgNLU6prrPnJmoHkSBPP3CJTkFgVkigEbMfXbd4i2mp8PTN_OMNFCjv-SSEVZsbNNt1yMDBcbYATQYyIFK63k7A',
    ...
    },
    {
      ...
      price: '$12.00',
      description:
        'Empaque al vacío con 50 unidades de salchicha tipo viena estándar. Larga duración.',
      imageUrl:
        'https://lh3.googleusercontent.com/aida-public/AB6AXuCtjIPGFwHKbCB9WVJ-opSccsbmZktXss9K_ruOrt4T4IgGn540li3OvT1CyGSw7LIc46VNpyxxsEjJm6LHKIKg8E_QnDewh7g6k5k7ZHeQxxdAZDUIac2uvVIXBwCoouE180Fe-HvYuPW13gB3zRHA_kHGV1WPdLGYwffDJOAT7VXDLbl_q5CY2UtDTrDHbeWCk41n7wXXnIAeD1vRotyuj79aw41K_6Nl3bRf6bsdWSln08t-mGT2p9etEKdbAHBy8dTPKyznxJ0',
  ...
  protected readonly branchOptions = this.branches.map((branch) => ({ label: branch.name, value: branch.id }));

  protected readonly currentHero = computed(() => this.heroSlides[this.activeHeroIndex()] ?? this.heroSlides[0]);
  ...
  protected readonly selectedBranch = computed(
    () => this.branches.find((branch) => branch.id === this.selectedBranchId()) ?? this.branches[0]
  );

  protected previousSlide(): void {
    this.activeHeroIndex.set((this.activeHeroIndex() + this.heroSlides.length - 1) % this.heroSlides.length);
  }

  protected nextSlide(): void {
    this.activeHeroIndex.set((this.activeHeroIndex() + 1) % this.heroSlides.length);
  }
